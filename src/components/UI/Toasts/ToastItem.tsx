...
export const ToastItem: React.FC<ToastItemProps> = ({ toast, onClose, timeout }) => {
	...
	return (
		<div
			className={cn(
				'flex items-center justify-between min-w-[300px] p-4 border-2 rounded-xl shadow-lg transition-all animate-in fade-in slide-in-from-right-4',
				styles[toast.type],
			)}
		>
			<span className="font-medium">{toast.message}</span>
			<button onClick={() => onClose(toast.id)} className="ml-4 hover:opacity-70 transition-opacity cursor-pointer">
